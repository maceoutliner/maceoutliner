...
from fiction_outlines.models import Series, Outline, Character, Location, Arc, ArcElementNode
...
class User(TimeStampedModel, AbstractUser):
    ...
    def prepare_history_query_set_dict(self, datetime_field='created'):
        query_set_dict = {
            ...
            'outlines': Outline.objects.all().select_related('series').prefetch_related(
                'tags', 'arc_set', 'storyelementnode_set'),
            'arcs': Arc.objects.all().select_related('outline').prefetch_related(
                'arcelementnode_set'),
            'characterinstances': CharacterInstance.objects.filter(character__user=self),
        }
        if datetime_field == 'created':
            query_set_dict['locationinstances'] = LocationInstance.objects.filter(location__user=self)
            query_set_dict['storynodes'] = StoryElementNode.objects.filter(outline__user=self).select_related(
                'outline').prefetch_related('assoc_characters', 'assoc_locations',
                                            'arcelementnode_set').exclude(story_element_type='root')
            query_set_dict['arcnodes'] = ArcElementNode.objects.filter(
                arc__outline__user=self).select_related(
                    'story_element_node').prefetch_related(
                        'assoc_characters',
                        'assoc_locations').exclude(arc_element_type='root')
    ...
    def get_all_recent_changes(self, num_results):
        ...
        sorted_changes = sorted(all_changes_unsorted, key=itemgetter('when'), reverse=True)
...
def update_outline_modify_based_on_story_node(sender, instance, created, *args, **kwargs):
    '''

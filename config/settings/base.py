...
if "DATABASE_URL" not in os.environ.keys() and "DATA_DB_USER" in os.environ.keys():
    if os.environ["DATA_DB_USER"] == "nanobox":
        os.environ["DATABASE_URL"] = "postgres://{0}:{1}@{2}/gonano".format(
...
)  # (maceoutliner/config/settings/base.py - 3 = maceoutliner/)
...
MACEOUTLINER_APPS = ["fiction_outlines", "fiction_outlines_api"]
...
# ------------------------------------------------------------------------------
MIGRATION_MODULES = {"sites": "maceoutliner.contrib.sites.migrations"}
...
DATABASES = {"default": env.db("DATABASE_URL", default="postgres:///maceoutliner")}
